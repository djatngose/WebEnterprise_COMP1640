...
namespace PopForums.Services
{
	public class TopicService : ITopicService
	{
        ...
        {
            return _topicRepository.GetTopicsInForum(forumId);
        }
        public int GetTopicsInCategory(int categoryId)
        {
            return _topicRepository.GetTopicsInCategory(categoryId);
        }
        public Post PostReply(Topic topic, User user, int parentPostID, string ip, bool isFirstInTopic, NewPost newPost, DateTime postTime, string topicLink, Func<User, string> unsubscribeLinkGenerator, string userUrl, Func<Post, string> postLinkGenerator)
		{
			...
			var postID = _postRepository.Create(topic.TopicID, parentPostID, ip, isFirstInTopic, newPost.IncludeSignature, user.UserID, user.Name, newPost.Title, newPost.FullText, postTime, false, user.Name, null, false, 0,newPost.IsAnonymous,null);
