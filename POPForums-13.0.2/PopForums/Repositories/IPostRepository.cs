...
namespace PopForums.Repositories
{
	...
	{
		int Create(int topicID, int parentPostID, string IP, bool isFirstInTopic, bool showSig, int userID, string name, string title, string fullText, DateTime postTime, bool isEdited, string lastEditName, DateTime? lastEditTime, bool isDeleted, int votes,bool isAnonymous,string fileUrl);
		bool Update(Post post);

	    List<Post> GetAll();
	    List<Post> GetAllComments();
         int GetReplyCountByStudent(int topicID, bool includeDeleted,int type);
        ...
        CountVote GetVoteCount(int postID);
        CountVote CalculateVoteCount(int postID);

        void SetVoteCount(int postID, int votes, int downVotes);
		void VotePost(int postID, int userID, bool isUp);
		Dictionary<int, string> GetVotes(int postID);
		List<CountVote> GetVotedPostIDs(int userID, List<int> postIDs);
