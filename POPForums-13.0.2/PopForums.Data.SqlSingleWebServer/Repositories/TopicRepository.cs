...
namespace PopForums.Data.SqlSingleWebServer.Repositories
{
	public class TopicRepository : ITopicRepository
	{
        ...
        internal const string TopicFieldsNew = "pf_Topic.TopicID, pf_Topic.ForumID, pf_Topic.Title, pf_Topic.ReplyCount, pf_Topic.ViewCount, pf_Topic.StartedByUserID, pf_Topic.StartedByName, pf_Topic.LastPostUserID, pf_Topic.LastPostName, pf_Topic.LastPostTime, pf_Topic.IsClosed, pf_Topic.IsPinned, pf_Topic.IsDeleted, pf_Topic.IsIndexed, pf_Topic.UrlName, pf_Topic.AnswerPostID,pf_Post.IsAnonymous";
...
ORDER BY pf_Topic.LastPostTime DESC")
		...
		public Topic Get(int topicID)
		{
			Topic topic = null;
			_sqlObjectFactory.GetConnection().Using(c =>
				c.Command("SELECT " + TopicFields + " FROM pf_Topic  WHERE pf_Topic.TopicID = @TopicID ")
        ...
        public Topic GetTopicsInForum(int forumId)
        {
            Topic forum = null;
            _sqlObjectFactory.GetConnection().Using(connection =>
                connection.Command("SELECT " + TopicFields + " FROM pf_Topic WHERE ForumID = @ForumId")
                    .AddParameter("@ForumId", forumId)
                    .ExecuteReader()
                    .ReadOne(r => forum = GetTopicFromReader(r)));
            return forum;
        }
        public int GetTopicsInCategory(int categoryId)
        {
            var count = 0;
            _sqlObjectFactory.GetConnection().Using(connection => count = Convert.ToInt32(
                connection.Command("SELECT COUNT(*) FROM pf_Topic WHERE ForumID IN (SELECT ForumID from pf_Forum where CategoryID = @CategoryId)")
...
WITH Entries AS ( 
SELECT ROW_NUMBER() OVER (ORDER BY pf_Topic.IsPinned DESC, pf_Topic.LastPostTime DESC)
...
pf_Topic.LastPostTime, pf_Topic.IsClosed, pf_Topic.IsPinned, pf_Topic.IsDeleted, pf_Topic.IsIndexed, pf_Topic.UrlName, pf_Topic.AnswerPostID ,pf_Post.IsAnonymous
FROM pf_Topic inner join pf_Post on pf_Topic.TopicID = pf_Post.TopicID WHERE pf_Topic.ForumID = @ForumID  and pf_Post.IsFirstInTopic =1
AND ((@IncludeDeleted = 1) OR (@IncludeDeleted = 0 AND pf_Topic.IsDeleted = 0)))

SELECT TopicID, ForumID, Title, ReplyCount, ViewCount, 
StartedByUserID, StartedByName, LastPostUserID, LastPostName, 
LastPostTime, IsClosed, IsPinned, IsDeleted, IsIndexed, UrlName, AnswerPostID,IsAnonymous
FROM Entries 
WHERE Row between 
...
SET ROWCOUNT 0";
			...
			_sqlObjectFactory.GetConnection().Using(connection => 
				connection.Command(sql)
					...
					.ExecuteReader()
					.ReadAll(r => topics.Add(GetTopicFromReaderNew(r))));
			return topics;
		}
...
FROM pf_Topic WHERE ForumID = @ForumID AND ((@IncludeDeleted = 1) OR (@IncludeDeleted = 0 AND IsDeleted = 0))";
        ...
        internal static Topic GetTopicFromReaderNew(IDataRecord reader)
        {
            return new Topic(reader.GetInt32(0))
            {
                ForumID = reader.GetInt32(1),
                ...
                UrlName = reader.GetString(14),
                AnswerPostID = reader.NullIntDbHelper(15),
                IsAnonymous =  reader.NullBoolDbHelper(16)
            };
        }
