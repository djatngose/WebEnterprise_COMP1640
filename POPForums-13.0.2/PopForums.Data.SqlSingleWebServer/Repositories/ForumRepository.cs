...
namespace PopForums.Data.SqlSingleWebServer.Repositories
{
	public class ForumRepository : IForumRepository
	{
        ...
        {
            var result = 0;
            _sqlObjectFactory.GetConnection().Using(connection => connection.Command("DELETE FROM pf_Forum WHERE ForumID = @ForumID")
                    .AddParameter("@ForumID", forumID)
                    .ExecuteNonQuery());
            //if (result != 1)
            //    throw new Exception(String.Format("Can't delete forum with ID {0} because it does not exist.", forumID));
        ...
        public int GetAggregatePostCountByStudent()
        {
            //var cacheObject = _cacheHelper.GetCacheObject<int?>(CacheKeys.AggregatePostCount);
            //if (cacheObject != null)
            //    return cacheObject.Value;
            ...
            _sqlObjectFactory.GetConnection().Using(connection =>
            {
                var result = connection.Command("SELECT COUNT(*) FROM pf_post a INNER JOIN pf_topic b ON a.TopicID = b.TopicID INNER JOIN dbo.pf_Forum c ON b.ForumID = c.ForumID inner join pf_PopForumsUser d on d.UserID = a.UserID WHERE ISNULL(a.IsDeleted, 0) = 0 and d.UserType=1")
                   .ExecuteScalar();
                if (result.GetType() == typeof(Int32))
                    count = Convert.ToInt32(result);
            });
